...
leds.Base.pad_L = [36,37,38,39,44,45,46,47,52,53,54,55,60,61,62,63]; //notes for the left half of base
leds.Base.pad_R = [40,41,42,43,48,49,50,51,56,57,58,59,64,65,66,67]; //notes for the Right half of base
...
function sxout(cmdbytes){
  var msg = [];
  msg = msg.concat(sysex.head,pid,cmdbytes,sysex.eom);
  clog("sysex out "+msg);
  midiOut.send(msg);
}
...
function light(nn,c,time,offcolor){
  if(nn != undefined){
    ...
    if(time){
      //clog('blink light '+nn+' '+c+' '+color[c]+' time '+time+' notemsg '+notemsg);
    ...
    clog('!!! >>> midi message error on note value '+nn)
  }
}
...
window.addEventListener('load', function() {   
  if (navigator.requestMIDIAccess){
    ...
    //alert that jazz is in use. The div 'MIDIPlugin' is generated by the WebMIDIAPI.js shim.
    var jazzcheck = document.getElementById("MIDIPlugin");
    if(jazzcheck){
      var j = document.getElementById("usingjazz");
    ...
    var gwarn =  document.getElementById('calert');
    var bwarn =  document.getElementById('otherbalert');
    warn.style.visibility = 'visible';
    warn.style.display = 'block';
    if(isChrome){
      gwarn.style.visibility = 'visible';
    }else{
      bwarn.style.visibility = 'visible';
    }
  }
